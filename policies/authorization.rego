...
allow_result := allow("workload.operation.allowed", 5) if { tenant_ok; operation_fresh; input.action in {"runs.claim", "runs.heartbeat", "runs.complete"}; input.agent.risk_class in {"low", "medium"}; has_role("trusted-workload") }
allow_result := allow("workload.operation.allowed", 0) if { tenant_ok; input.security_state.authoritative; input.action in {"runs.claim", "runs.heartbeat", "runs.complete"}; input.agent.risk_class in {"high", "critical"}; has_role("trusted-workload") }
allow_result := allow("workload.operation.allowed", 0) if { tenant_ok; input.security_state.authoritative; input.action == "resources.settle"; has_role("trusted-settler") }
allow_result := allow_with_obligations("workload.operation.allowed", 0, [budget_exhaustion_obligation]) if { tenant_ok; input.security_state.authoritative; input.action == "resources.meter"; has_role("trusted-meter") }
