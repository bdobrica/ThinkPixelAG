...
func TestVerifyIncludesRequiredNonContainerGates(t *testing.T) {
	...
	for _, required := range []string{"generate-check", "lint", "test", "test-race", "test-policy", "test-integration", "test-e2e", "test-security", "compose-check", "kubernetes-check", "security", "build", "container-smoke"} {
		found := false
