...
- [x] DATA-002 Create initial migrations for tenants/principals, agents/versions/capabilities/approvals, policies/activations, and their integrity constraints/indexes. — completed 2026-08-19, commit `pending`, evidence: migration source/schema contract tests; PostgreSQL 18.4 apply/reverse rehearsal; `make verify`; `docs/architecture/database-schema.md`
- [x] DATA-003 Add migrations for runs, run version snapshots, state versions, signals, and append-only ordered run events. — completed 2026-08-19, commit `pending`, evidence: migration source/schema contract tests; PostgreSQL 18.4 apply/reverse rehearsal; `make verify`; `docs/architecture/database-schema.md`
- [x] DATA-004 Add migrations for resource dimensions, envelopes, balances, reservations, trusted usage ledger, settlements, and uniqueness/check constraints. — completed 2026-08-19, commit `pending`, evidence: migration source/schema contract tests; PostgreSQL 18.4 apply/reverse rehearsal; `make verify`; `docs/architecture/database-schema.md`
- [x] DATA-005 Add migrations for revocations, monotonic epochs, ordered revocation log, and gateway checkpoints. — completed 2026-08-19, commit `pending`, evidence: migration source/schema contract tests; PostgreSQL 18.4 apply/reverse rehearsal; `make verify`; `docs/architecture/database-schema.md`
- [x] DATA-006 Add migrations for idempotency records, audit events, and transactional outbox claim/retry/dead-letter fields. — completed 2026-08-19, commit `pending`, evidence: migration source/schema contract tests; PostgreSQL 18.4 apply/reverse rehearsal; `make verify`; `docs/architecture/database-schema.md`
...
| 2026-08-19 | DATA-002–DATA-006 | `pending` | contiguous migration and schema-contract tests; PostgreSQL 18.4 apply/reverse rehearsal; `make verify`; hardened container smoke | Composite tenant foreign keys prevent cross-tenant relationships; global ordered streams are indexed separately; repository/role immutability enforcement remains DATA-007/DATA-008 as documented |
