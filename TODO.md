...
- [x] REG-003 Implement approval/deprecation/revocation eligibility state and authorized admin endpoints with evidence records. — completed 2026-08-22, commit pending, evidence: lifecycle/application/HTTP authorization unit and race tests; migration 007 checksum/schema tests; PostgreSQL 18.4 concurrent transition, tenant isolation, atomic audit/outbox, and append-only integration tests; OpenAPI lint; `make verify`
...
| 2026-08-22 | REG-003 | pending | lifecycle/application/HTTP authorization unit and race tests; migration 007 checksum/schema tests; PostgreSQL 18.4 concurrent transition, tenant isolation, atomic audit/outbox, and append-only integration tests; OpenAPI lint; `make verify` | Decisions require an authoritative `versions.approve` policy ALLOW; version-row locking serializes competing transitions; decision, audit, and outbox evidence commit atomically |
