...
- [x] POL-004 Add Rego unit/golden tests for allow/deny, malformed input/output, policy outage, stale state, cross-tenant access, and non-expansion of constraints. — completed 2026-08-20, commit `pending`, evidence: pinned OPA 1.19.0 17-case Rego suite; malformed/mismatched/unknown OPA output and outage race tests; `make verify`
- [x] POL-005 Add optional Valkey-backed decision caching keyed by policy version, relevant epochs, subject/action/resource/input digest, with bounded TTL and safe bypass on failure. — completed 2026-08-20, commit `pending`, evidence: normalized hashed cache-key and TTL/freshness tests; HMAC-authenticated RESP adapter; Valkey 9.1.1 auth/expiry/integrity integration; failure/poison bypass race tests; `make verify`
