...
- [x] ENG-002 Add dependency policy, license/source checks, vulnerability scanning configuration, and reproducible tool pinning. — completed 2026-08-10, commit `PENDING`, evidence: dependency checker unit/integration tests; pinned `govulncheck` and `go-licenses` gates; `go test ./...`; `git diff --check`
...
| 2026-08-10 | ENG-002 | `PENDING` | source-policy audit; `govulncheck -test ./...`; `go-licenses check --include_tests`; root/tools tests; `git diff --check` | No dependency exceptions; Make and CI wiring remain ENG-008/ENG-010 |
