...
| 2026-08-10 | ENG-008 | `PENDING` | Makefile contract tests; exact-version tool, generation/format/lint, unit/race/policy/integration/e2e, security, build, and aggregate verification evidence | `image` intentionally requires ENG-011's Dockerfile and remains outside `verify` until then; OPA/dependency-backed suites report their scheduled absence explicitly |
