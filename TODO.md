...
- [x] ENG-010 Add CI for formatting, generation drift, lint, unit/race tests, policy tests, integration tests, build, vulnerability/license checks, and image build. — completed 2026-08-10, commit `PENDING`, evidence: eight least-privilege GitHub Actions jobs; immutable action pins; CI contract tests; `make verify`
...
| 2026-08-10 | ENG-010 | `PENDING` | CI contract tests; formatting/generation/lint, unit/race/policy/integration, dependency/vulnerability/license, binary and image jobs; `make verify` | Third-party actions use full commit SHAs; image job reports the ENG-011 prerequisite and automatically activates `make image` when `Dockerfile` exists |
