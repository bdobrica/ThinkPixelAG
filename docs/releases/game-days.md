...
|---|---|---|
| Install and restricted runtime | Disposable Kubernetes installation/migration/disruption at `4fff867`; retained posture and RC-002 live smoke | [Lifecycle evidence](../operations/ops012-evidence.md); not every production distribution |
...
| Backup/restore, PITR and forward recovery | Encrypted PostgreSQL 18.4 physical base backup plus WAL; exact pre/post-governance targets; schema 17→18; epoch/accounting/evidence invariants | [OPS-009 evidence](../phase-8-evidence.md), source `fa04a0c`; selected-target RPO zero, local RTO 3–4 seconds; not a provider SLA |
| Durable process recovery | SSD PostgreSQL crash with 42-table content comparison; sink/exporter crash and exact receipt-chain replay | [Database](../operations/ssd-qualification.md) and [sink](../operations/ssd-evidence-qualification.md); one SSD host, not host-loss HA |
...
| Revocation reconciliation | Real mTLS test consumer disconnected 31 seconds; freshness denied; authoritative reconciliation restored monotonic state | [OPS-011 partition](../operations/ops011-evidence.md), plus fresh component gap/snapshot tests; production gateway remains DQ-003 |
...
in the [existing deferral register](../operations/deferred-qualification.md).
