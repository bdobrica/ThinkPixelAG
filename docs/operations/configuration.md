...
dependency checks after startup.
...
| `THINKPIXELAG_VALKEY_CACHE_HMAC_KEY` | none | unset | required when Valkey is enabled; secret of at least 32 bytes used to authenticate cache entries, including runtime throughput hints |
...
| `THINKPIXELAG_RUNTIME_FILE` | none | unset | mounted runtime JSON; enables governed routes |
| `THINKPIXELAG_CURSOR_HMAC_KEY` | none | unset | at least 32 bytes when runtime file is set; invalid without runtime file |
| `THINKPIXELAG_SIGNING_PROVIDER` | `--signing-provider` | `disabled` | `disabled`, `kms`, `hsm`; production requires `kms` or `hsm` |
| `THINKPIXELAG_SIGNING_KEY_ID` | `--signing-key-id` | unset | managed provider requires an opaque key reference, never a private key/file path |
| `THINKPIXELAG_SIGNING_ALGORITHM` | `--signing-algorithm` | `ED25519` | `ED25519`, `ECDSA_SHA256`, `RSA_PSS_SHA256` |
| `THINKPIXELAG_EVIDENCE_SINK_ID` | `--evidence-sink-id` | unset | stable sink ID, 1–256 canonical bytes; required with export |
| `THINKPIXELAG_EVIDENCE_ENDPOINT` | `--evidence-endpoint` | unset | HTTPS, even locally; required with export |
| `THINKPIXELAG_EVIDENCE_BEARER_TOKEN` | none | unset | required with export; secret with no line breaks |
| `THINKPIXELAG_EVIDENCE_TIMEOUT` | `--evidence-timeout` | `5s` | greater than zero, at most `5m` |
| `THINKPIXELAG_EVIDENCE_MAX_RESPONSE_BYTES` | `--evidence-max-response-bytes` | `65536` | receipt bound from 1 through 1048576 when export is enabled |

Durations use Go syntax such as `500ms`, `5s`, or `2m`.

## Secret handling

Database and Valkey URLs, cache/cursor HMAC keys, and OPA/evidence bearer tokens
are environment-only. They are
...
# Also deliver THINKPIXELAG_VALKEY_CACHE_HMAC_KEY (at least 32 bytes) if enabling it.
...
When sink ID, HTTPS endpoint and bearer token are supplied together, a background exporter
